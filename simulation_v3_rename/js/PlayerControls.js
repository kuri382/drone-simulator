THREE.PlayerControls = function ( camera, player, domElement ) {
	...
	var thetaDelta = 0;
	var scale = 0.8;//カメラ距離
	var angle_watch_down = 10;//カメラ角度xy
	var x_init = 3;
	var y_init = 1;
	var z_init = 3;
	...
	this.init = function() {
		...
		this.camera.position.x = this.player.position.x + x_init;
		this.camera.position.y = this.player.position.y + y_init;
		this.camera.position.z = this.player.position.x + z_init;
	...
	this.update = function() { 
		...
		if ( this.autoRotate ) {
			this.camera.position.x += this.autoRotateSpeed * ( ( this.player.position.x + angle_watch_down * Math.sin( this.player.rotation.y ) ) - this.camera.position.x );
			this.camera.position.z += this.autoRotateSpeed * ( ( this.player.position.z + angle_watch_down * Math.cos( this.player.rotation.y ) ) - this.camera.position.z );
	...
	this.checkKeyStates = function () {
		...
		if ( keyState[32] ) {
	        ...
	        //this.player.position.y -= this.moveSpeed * Math.sin( this.player.rotation.y );

	        this.camera.position.y += this.moveSpeed * Math.cos( this.player.rotation.y );
	        //this.camera.position.y -= this.moveSpeed * Math.sin( this.player.rotation.y );
